...
const MobileHeader: React.FC<MobileHeaderProps> = ({ onNavigate, onLogout, userProfile }) => {
    ...
    return (
        <header className="flex desktop:hidden bg-white border-b border-slate-100 p-4 sticky top-0 z-50 items-center justify-between shadow-sm no-print">
            <div
                ...
                onClick={() => onNavigate(isAdmin ? View.DASHBOARD : (userProfile?.access ? View.SCAN_COUPON : View.PROFILE))}
            >
                <div className="w-8 h-8 bg-whiterounded-lg flex items-center justify-center font-bold text-sm">
